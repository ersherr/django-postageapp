	...
	def send_messages(self, email_messages):
		...
		for message in email_messages:
			...
			if(getattr(message, "attachments")):
				file_name = message.attachments[0][0]
				file_string = message.attachments[0][1].encode("base64")
				attachment_dict = {"attachments":{file_name:{"content_type":"application/octet-stream","content":file_string}}}
				message_dict['arguments'].update(attachment_dict)
